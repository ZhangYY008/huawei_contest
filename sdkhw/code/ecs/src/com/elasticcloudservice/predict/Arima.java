...
public class Arima
{
	...
	public Arima(double [] primaryData)
	{
		...
		for(int i=0; i<preZscoreProcess.length; i++){
			if(preZscoreProcess[i]>=2.3 || preZscoreProcess[i]<=-2.3){//对于噪声点使用前后平均值替代
				//continue;
			//	if(i<1){
...
//		this.primaryData = primaryData;
	...
	public double [] firDiffOfPreProcess(double [] preData)		//一阶差分(1)
	{	
	...
	public double [] seasonDiffOfPreProcess(double [] preData)		//季节性差分(6, 7)
	{	
	...
	{
		if (period >= primaryData.length - 1)		// 将6也归为季节性差分
		{
			period = 0;
		}
		switch (period)
